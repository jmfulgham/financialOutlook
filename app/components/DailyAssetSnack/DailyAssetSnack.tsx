...
const DailyAssetSnack = memo(({ ticker }: DailyAssetCardProps) => {
  ...
  const isStockDown: boolean | undefined =
    stockData && stockData.open < stockData.close;
  useEffect(() => {
  ...
  return (
    <div
      className={"daily-asset-snack-container"}
    ...
    >
      ...
      {stockData && (
        <p className={"close-date"}>Close Date: {stockData.from}</p>
      )}
      {stockData && (
        <div className={"price-details"}>
          <p>Close Price: ${stockData.close}</p>
          {isStockDown ? (
